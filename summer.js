...
require(["fitText", "fitImage", "Loader", "mousetrap.min"], function (fitText, fitImage, Loader, Mousetrap) {
	...
	var state = new StateMachine({
		...
		"methods": {
			...
			"onMenu": function () {
				fitText(context, "Card Golf", 0, canvas.height/10, canvas.width, canvas.height/5);
				...
				game.images.cards = loader.newImageAsset("summer_vacation.jpg", function () {
					...
					for (var i=0; i<deck.length; i++) {
						fitImage(context, game.images.cards, 
							...
							Math.floor(i/10)*canvas.height/4 + canvas.height*.3,
							canvas.width/10,
							canvas.height/2);
					}
					onload();
