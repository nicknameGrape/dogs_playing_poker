...
require(["fitText", "fitImage", "Loader", "mousetrap.min", "dogs/janken"], function (fitText, fitImage, Loader, Mousetrap, playJanken) {
	...
	var state = new StateMachine({
		...
		"methods": {
			...
			"onMenu": function () {
				fitText(context, "We have a big castle.", 0, 0, canvas.width, canvas.height*.1);
				...
				game.images.cards = loader.newImageAsset("uno_town.jpg", function () {
					...
					for (var i=0; i<deck.length; i++) {
						fitImage(context, game.images.cards, 
							...
							subimageHeight,
							(i%4) * canvas.width/4,
							Math.floor(i/4)*canvas.height*.3 + canvas.height*.1,
							canvas.width/4,
							canvas.height/3);
					}
					onload();
				...
				function renderHand() {
					...
					for (var i=0; i<pile.length; i++) {
						fitImage(context, game.images.cards, 
							...
							subimageWidth,
							subimageHeight,
							(.35 + .01 * (i % 3)) * canvas.width,
							(.40 - .015 * i) * canvas.height,
					...
					for (var i=0; i<cards.length; i++) {
						fitImage(context, game.images.cards, 
							...
							subimageWidth,
							subimageHeight,
							progress * 2.5 * canvas.width + i * .5 * canvas.height / Math.sqrt(2),
							canvas.height * .7,
							canvas.width/4,
							canvas.height * .3
						);
