...
require(["fitText", "fitImage", "Loader", "mousetrap.min"], function (fitText, fitImage, Loader, Mousetrap) {
	...
	var state = new StateMachine({
		...
		"methods": {
			...
			"onMenu": function () {
				fitText(context, "Card Game: Fish War", 0, canvas.height/10, canvas.width, canvas.height/5);
				...
				game.images.cards = loader.newImageAsset(CARDS_SOURCE, function () {
					...
					for (var i=0; i<deck.length; i++) {
						fitImage(context, game.images.cards, 
							...
							Math.floor(i/(deck.length / 2))*canvas.height/4 + canvas.height*.3,
			...
			"onTable": function () {
				...
				Mousetrap.bind("space", function () {
					if (deck.length > 0) {
						...
						if (game.players[0].hand == null) {
							game.players.forEach(function (e) {
								if (e.cards.length > 0) {
									e.hand = e.cards.splice(Math.floor(Math.random() * e.cards.length), 1)[0];
								}
							});
						} else {
							var topCard = -1;
							...
							game.players.forEach(function (e, i) {
								if (i !== winner) {
									game.players[winner].pointCards.push(e.hand);
									e.hand = null;
								}
							});
							game.players[winner].pointCards.push(topCard);
							game.players[winner].hand = null;
						}
						renderTable();
					}
				});
				...
				function renderTable() {
					...
					game.players.forEach(function (element, index, array) {
						...
						for (var i=0; i<player.pointCards.length; i++) {
							fitImage(context, game.images.cards, 
								player.pointCards[i] * subimageWidth,
								0,
								subimageWidth,
								subimageHeight,
								canvas.width * (-.09),
						...
						//hand
						if (element.hand !== null) {
							fitImage(context, game.images.cards, 
								player.hand * subimageWidth,
								0,
								subimageWidth,
								subimageHeight,
								canvas.width * (-.05),
