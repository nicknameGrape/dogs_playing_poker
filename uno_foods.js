...
require(["fitText", "fitImage", "Loader", "mousetrap.min", "dogs/janken"], function (fitText, fitImage, Loader, Mousetrap, playJanken) {
	...
	var state = new StateMachine({
		...
		"methods": {
			...
			"onMenu": function () {
				fitText(context, "How does it taste?", 0, canvas.height/10, canvas.width, canvas.height/5);
				...
				game.images.cards = loader.newImageAsset("foods_described.jpg", function () {
					...
					for (var i=0; i<deck.length; i++) {
						fitImage(context, game.images.cards, 
							...
							subimageWidth,
							subimageHeight,
							(i%10) * canvas.width/10,
							Math.floor(i/10)*canvas.height/3 + canvas.height*.3,
							canvas.width/10,
							canvas.height/4);
					}
					onload();
