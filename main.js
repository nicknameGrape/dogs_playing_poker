	...
	var state = new StateMachine({
		...
		"methods": {
			...
			"onMenu": function () {
				...
				game.images.months = loader.newImageAsset("months.jpg", function () {
					subimageWidth = game.images.months.width / 24;
					subimageHeight = game.images.months.height;
					for (var i=0; i<=24; i++) {
						fitImage(context, game.images.months, 
							i * subimageWidth,
							0,
							subimageWidth,
							subimageHeight,
							i%6 * canvas.width/6,
							Math.floor(i/6) * canvas.height/2,
							canvas.width/6,
							canvas.height/2);
					}
					onload();
			...
			"onTable": function () {
				...
				Mousetrap.bind("space", function () {
					if (deck.length > 0) {
						...
						Mousetrap.bind("j", function () {playJanken(); renderTable()})
					}
				});
