...
require(["fitText", "fitImage", "Loader", "mousetrap.min"], function (fitText, fitImage, Loader, Mousetrap) {
	...
	var state = new StateMachine({
		...
		"methods": {
			...
			"onMenu": function () {
				fitText(context, "My Day", 0, canvas.height/10, canvas.width, canvas.height/5);
				...
				game.images.cards = loader.newImageAsset("routine.jpg", function () {
						...
						Mousetrap.bind("space", function () {
							...
							for (var i=0; i<7; i++) {
								for (var j=0; j<game.players.length; j++) {
									if (game.players[j].cards.indexOf(i) !== -1) {
										found = true;
										playerIndex = j;
										break;
									}
							...
							if (cardIndex == null) {
								...
								console.log(playerIndex, cardIndex);
							}
							renderTable();
						});
					}
